...
privsTopics.get = async function (tid, uid) {
	...
	const readable = privData['topics:read'] && (!(topicData.private === 'true') || (isAdminOrMod || isOwner));
...
privsTopics.filterTids = async function (privilege, tids, uid) {
	...
	const topicsData = await topics.getTopicsFields(tids, ['tid', 'cid', 'deleted', 'scheduled', 'private']);
	...
	// (!t.private || privsTopics.isOwnerOrAdminOrMod(t.tid, uid)) &&
